<article <?php hybrid_post_attributes(); ?>>
		...
		<header class="entry-header">
			...
			<?php echo apply_atomic_shortcode( 
				...
				'<div class="entry-byline">' . 
					hybrid_entry_terms_shortcode( array( 'taxonomy' => 'portfolio', 'before' => __( 'Работы:', 'eino' ) . ' ' ) ) . 
				'</div>'
			); ?>
		</header><!-- .entry-header -->
	...
	<?php } else { ?>
		...
		<div class="entry-summary">
			...
			<?php the_excerpt(); ?>
			<p class="eino-portfolio-item"><?php echo hybrid_entry_terms_shortcode( array( 'taxonomy' => 'portfolio', 'before' => __( 'Работы:', 'eino' ) . ' ' ) ); ?></p>
			<?php wp_link_pages( array( 'before' => '<p class="page-links">' . '<span class="before">' . __( 'Страницы:', 'eino' ) . '</span>', 'after' => '</p>' ) ); ?>
		</div><!-- .entry-summary -->
