...
<div class="loop-meta">
	...
	<?php } elseif ( is_search() ) { ?>
		...
		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'Результаты поиска: "%s"', 'eino' ), esc_attr( get_search_query() ) ) ); ?>
		</div><!-- .loop-description -->
	...
	<?php } elseif ( is_day() || is_month() || is_year() ) { ?>
		...
		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'Результаты поиска: %s.', 'eino' ), $date ) ); ?>
		</div><!-- .loop-description -->

	<?php } elseif ( is_archive() ) { ?>

		<h1 class="loop-title"><?php _e( 'Архив', 'eino' ); ?></h1>

		<div class="loop-description">
			<?php echo wpautop( __( 'Вы смотрите архив.', 'eino' ) ); ?>
		</div><!-- .loop-description -->
