<article <?php hybrid_post_attributes(); ?>>
	...
	<?php if ( is_singular( get_post_type() ) ) { ?>
		...
		<header class="entry-header">
			...
			<?php echo apply_atomic_shortcode( 'entry_byline', '<div class="entry-byline">' . __( 'Цена:', 'eino' ) . ' ' . eino_edd_the_price( get_the_ID() ) . '</div>' ); ?>
		</header><!-- .entry-header -->
	...
	<?php } else { ?>
		...
		<div class="entry-summary">
			...
			<?php } else { ?>
				<p class="eino-download"><?php  echo __( 'Цена:', 'eino' ) . ' ' . eino_edd_the_price( get_the_ID() ) ?></p>
			<?php } ?>
			<?php wp_link_pages( array( 'before' => '<p class="page-links">' . '<span class="before">' . __( 'Страницы:', 'eino' ) . '</span>', 'after' => '</p>' ) ); ?>
		</div><!-- .entry-summary -->
