...
function hybrid_comment_form_args( $args ) {
	...
	$fields = array(
		...
		'email'  => '<p class="form-email' . esc_attr( $input_class ) . '"><label for="email">' . __( 'Почта', 'hybrid-core' ) . $req . '</label> <input type="email" class="text-input" name="email" id="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="40" /></p>',
		'url'    => '<p class="form-url"><label for="url">' . __( 'Сайт', 'hybrid-core' ) . '</label><input type="url" class="text-input" name="url" id="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="40" /></p>'
	);

	/* Sets the default arguments for displaying the comment form. */
	$args = array(
		'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
		'comment_field'        => '<p class="form-textarea req"><label for="comment">' . __( 'Комментировать', 'hybrid-core' ) . '</label><textarea name="comment" id="comment" cols="60" rows="10"></textarea></p>',
		...
		'title_reply_to'       => __( 'Комментировать запись %s', 'hybrid-core' ),
		'cancel_reply_link'    => __( 'Отменить', 'hybrid-core' ),
		'label_submit'         => __( 'Комментировать', 'hybrid-core' ),
	);
