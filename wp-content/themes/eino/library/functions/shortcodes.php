...
function hybrid_wp_link_shortcode() {
	return '<a href="http://wp-templates.ru/" title="скачать шаблон">шаблоны сайтов</a>';
}
...
function hybrid_theme_link_shortcode() {
	...
	return '<a href="http://searchtimes.ru/" title="seo форум">настройка wordpress</a>';
}
...
function hybrid_child_link_shortcode() {
	$theme = wp_get_theme();
	return '<a href="http://searchtimes.ru/">настройка wordpress</a>';
}
...
function hybrid_entry_edit_link_shortcode( $attr ) {
	...
	return $attr['before'] . '<span class="edit"><a class="post-edit-link" href="' . esc_url( get_edit_post_link( get_the_ID() ) ) . '" title="' . sprintf( esc_attr__( 'Изменить %1$s', 'hybrid-core' ), $post_type->labels->singular_name ) . '">' . __( 'Изменить', 'hybrid-core' ) . '</a></span>' . $attr['after'];
}
...
function hybrid_entry_comments_link_shortcode( $attr ) {
	...
	$attr = shortcode_atts( 
		array( 
			'zero'      => __( 'Комментировать', 'hybrid-core' ), 
			'one'       => __( 'Один комментарий', 'hybrid-core' ), 
			'more'      => __( 'Комментариев: %1$s', 'hybrid-core' ), 
	...
	elseif ( 0 == $number )
		$comments_link = '<a class="' . esc_attr( $attr['css_class'] ) . '" href="' . get_permalink() . '#respond" title="' . sprintf( esc_attr__( 'Прокомментировать запись %1$s', 'hybrid-core' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $attr['zero'], number_format_i18n( $number ) ) . '</a>';
	elseif ( 1 == $number )
		$comments_link = '<a class="' . esc_attr( $attr['css_class'] ) . '" href="' . get_comments_link() . '" title="' . sprintf( esc_attr__( 'Прокомментировать запись %1$s', 'hybrid-core' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $attr['one'], number_format_i18n( $number ) ) . '</a>';
	elseif ( 1 < $number )
		$comments_link = '<a class="' . esc_attr( $attr['css_class'] ) . '" href="' . get_comments_link() . '" title="' . sprintf( esc_attr__( 'Прокомментировать запись %1$s', 'hybrid-core' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $attr['more'], number_format_i18n( $number ) ) . '</a>';
...
function hybrid_comment_edit_link_shortcode( $attr ) {
	...
	$link = '<a class="comment-edit-link" href="' . esc_url( $edit_link ) . '" title="' . sprintf( esc_attr__( 'Изменить %1$s', 'hybrid-core' ), $comment->comment_type ) . '"><span class="edit">' . __( 'Изменить', 'hybrid-core' ) . '</span></a>';
...
function hybrid_comment_reply_link_shortcode( $attr ) {
	...
	$defaults = array(
		'reply_text' => __( 'Ответить', 'hybrid-core' ),
		'login_text' => __( 'Войти для комментирования.', 'hybrid-core' ),
