...
function hybrid_get_default_theme_settings() {
	...
	if ( is_array( $supports[0] ) && in_array( 'footer', $supports[0] ) && current_theme_supports( 'hybrid-core-shortcodes' ) ) {
		...
		if ( is_child_theme() )
			$settings['footer_insert'] = '<p class="copyright">' . __( 'Все права защищены &#169; [the-year] [site-link].', 'hybrid-core' ) . '</p>' . "\n\n" . '<p class="credit">' . __( '[wp-link], [theme-link], [child-link].', 'hybrid-core' ) . '</p>';

		/* If no child theme is active, leave out the [child-link] shortcode. */
		else
			$settings['footer_insert'] = '<p class="copyright">' . __( 'Все права защищены &#169; [the-year] [site-link].', 'hybrid-core' ) . '</p>' . "\n\n" . '<p class="credit">' . __( '[wp-link]  [theme-link].', 'hybrid-core' ) . '</p>';
	}
